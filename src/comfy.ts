...
export async function render(opts: RenderOptions): Promise<RenderResult> {
  ...
  const width = opts.width ?? (model === "qwen" ? 1536 : 1024);
  const height = opts.height ?? (model === "qwen" ? 1024 : 1024);

  const graph = model === "qwen"
    ? qwenGraph({ positive: opts.positive, negative: opts.negative ?? "", prefix: opts.prefix, width, height, steps: opts.steps ?? 36, cfg: opts.cfg ?? 2.5, shift: opts.shift ?? 3.1, seed })
    : fluxGraph({ positive: opts.positive, prefix: opts.prefix, width, height, steps: opts.steps ?? 4, seed });
